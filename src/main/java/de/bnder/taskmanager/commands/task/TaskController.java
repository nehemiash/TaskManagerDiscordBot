...
public class TaskController implements Command {
    ...
    public void action(String[] args, String messageContentRaw, Member commandExecutor, TextChannel textChannel, Guild guild, List<Member> mentionedMembers, List<Role> mentionedRoles, List<TextChannel> mentionedChannels, SlashCommandEvent slashCommandEvent) throws IOException {
        if (args[0].equalsIgnoreCase("edit") || args[0].equalsIgnoreCase("e")) {
            ...
            if (NumberUtils.isCreatable(formatPossibleTaskID(taskID)) && new Task(taskID, guild).exists()) {
                newTextStartIndex = 3;
            } else {
            ...
            if (!(taskID.equalsIgnoreCase("done") || (NumberUtils.isCreatable(formatPossibleTaskID(taskID)) && new Task(taskID, guild).exists()))) {
                String tempTaskID = getTaskIDFromContext(textChannel, commandExecutor, messageContentRaw);
                if (NumberUtils.isCreatable(tempTaskID) && new Task(tempTaskID, guild).exists()) {
                    taskID = tempTaskID;
                }
     ...
     *
     * @param textChannel     The textchannel where the command was executed.
